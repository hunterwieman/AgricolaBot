...
| `joint_taper128_thin` | 2026-06-15 | 2 | 3 | **117k self-play, snapshot-thinned** (1/6 of the old 57k + 1/2 of the new `joint_taper57k_selfplay_60k`) | shared trunk `[256,256]→128`, dropout 0.2, wd 1e-4; **int8 store + per-dir snapshot-keep** (fits the 8 GB M1, ~80 s/epoch vs 1100 s thrash); bs 8192, all cores; warm trunk (full) from `joint_taper128_117k`/epoch-25 | ~6.8M (value train rows) | 2.726 (value val — **low-variance thin val, not cross-comparable**) | **NEW STRONGEST; first joint model to clear the *objective* ensemble yardstick.** best **epoch 55** (early-stop, 70). **800-sim PUCT (C++): beats `joint_taper128_57k` 84-15-1 (+3.68) t0 / 86-13-1 (+3.65) t03.** **vs the 8-config heuristic ensemble (Python PUCT): ~100%** — e.g. champion `alphas_gen_7` **30-0 (+23.77)**, `t2` 4-0 — scoring ~2.4× the heuristics' points. Winning *both* the head-to-head AND the uncontaminated ensemble = real strength, not self-play exploitation. Validated the **warm-start normalization fix** (`target_std` no longer transplanted; the `value_scale`-measurement `NameError` fixed). **`value_scale` caveat: the stored 3.019 is a low-variance-val artifact — `value_scale` is distribution-dependent; measure on a COMMON state set for fair matches (common ≈ 6.25, used above).** **PROMOTED to `nn_models/best` (2026-06-15)** — the `best` pointer now resolves to this joint model via the `model_kind`-aware `load_value_evaluator`; the promoted `best.meta.json` carries **`value_scale = 6.25`** (the common-distribution value, NOT the stored thin-val 3.019 — see the per-model details below for the measured spread). See SHARED_TRUNK.md. |
