...
| `joint_outcome_44k` | 2026-06-22 | 2 | 3 | **44.6k self-play (1600 sims)** — 40k cloud `selfplay_1600_40k_cloud` (seed 100100000) + 4.6k local `selfplay_1600_40k` (seed 100000000, traces replayed) | shared trunk `[256,256]→128`, dropout 0.2, wd 1e-4; bs 2048; lr 3e-4; **L2-SP λ=1e-3 (excludes the fresh outcome head)**; **warm (full, 42 tensors) from champion `exp_visit_combined`/best.pt**; **NEW joint value+outcome: a separate `outcome_head` (E→1, target sign(margin)∈{-1,0,1}) co-trained in the value batch off the SAME embedding (one trunk forward) — margin head unchanged** | 44,608 games | margin head 2.59 val_mae + **0.69 outcome sign-acc** (low-variance self-play val, not cross-comparable) | **CURRENT CHAMPION; PROMOTED to `nn_models/best` + `cpp_export_best` (2026-06-23), deployed with the MIX leaf at α=0.9 (90% margin / 10% outcome).** First GCP cloud-trained model. best **epoch 12** (early-stop, 20 epochs); value_scale 2.943 (training proxy); deployment scales in the export manifest = value_scale 2.966 + outcome_scale 0.533 (common-state). Warm-start preserved value/policy (flat across epochs). **1000-game head-to-head vs prior champion `exp_visit_combined` (seat-balanced, common-state value_scale): beats it at deployment depths — margin/outcome/mix = 53.1/54.1/55.2% @ 800 sims, 56.1/56.4/59.0% @ 1600; ~tie @ 200.** (An initial 400-game run overstated this; fresh-seed re-run corrected it.) **Mix-rate α-sweep (10k games, kernel regression): win-prob-vs-α peaks at α≈0.9 (margin-heavy) vs a uniform-random-α self-field — pure outcome worst, 0.5 mix mediocre. So the mix/outcome edge over the champion is partly champion-specific exploitation while margin is the robust general leaf → deployed α=0.9 (mostly margin + a small outcome nudge).** C++ leaf modes (margin/outcome/mix) + tunable α + `--move`/`--analyze` `--leaf-mode` + the `mix` analysis badge (raw un-denormalized Q) all landed. Distinct from `pooled_all_thin_outcome` (value head itself regressed outcome) — this keeps margin AND adds a separate outcome head. |
