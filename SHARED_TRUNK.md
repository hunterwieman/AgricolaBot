...
### 2.1 Architecture variants (available; experimental)

Two optional architecture variants are wired in as available knobs — both are
experiments under evaluation, not promoted defaults.

- **Leaky-ReLU activation.** Added to the Python model's `_ACTIVATIONS` registry
  and selectable via `train_shared.py --activation leaky_relu` (the trunk and heads
  default to GELU otherwise). The activation is **also ported into the C++
  hand-rolled MLP**: the C++ inference reads a **model-global `activation` field
  from the export manifest** (default `gelu`, for backward compatibility with
  pre-activation-field manifests), so an exported leaky-ReLU model runs natively in
  C++ self-play/eval. A permanent differential gate
  (`test_cpp_joint_leaky_matches_python`) validates the C++ leaky-ReLU path ≤1e-4
  against Python.
- **Siamese per-player encoder — `SiameseSharedTrunkModel`.** A drop-in subclass of
  `SharedTrunkModel` (CLI `--siamese`, `--player-encoder-dims`,
  `--player-encoder-out`) where **both players' feature blocks run through one
  *shared* per-player encoder**, then `[emb_own ; emb_opp ; board ; mid]` feeds the
  usual trunk + heads. The intent is to give the two symmetric player blocks a tied
  representation. The standard (non-Siamese) path is **byte-identical when
  `--siamese` is absent**. The C++ inference engine supports it (the manifest
  carries a `siamese` flag + the player-encoder blob; the embed path slices
  own/opp, runs each through the shared encoder, concats, then the existing
  trunk), validated by the permanent gate `test_cpp_joint_siamese_matches_python`
  (≤1e-4 vs Python).

### 2.2 The clean-300k architecture sweep

A 6-architecture sweep on a fresh **clean 300k-game corpus** (`gen300k/{t1,t15,t2,t3}`
— 1600 sims, c_uct 2, mixed temps, generated by the champion `joint_gelu_rand_240k`;
**snapshot-keep 0.5**, all GELU random-init): **A** `[256,256]→128` (= champion arch),
**B_wide** `[512,512]→256`, **C_deep** `[256,256,256]→128`, **F_compact** `[128,128]→64`,
and two siamese variants **D** (54→128→64 player encoder) and **E** (54→256→128).

A **mid-training** 800-sim round-robin (500 games/pair, mix leaf α=0.9, champion scales,
no seat-swap) ranked them **B_wide 66.4% > C_deep 54.0% > A 52.7% > E_siam 48.4% >
D_siam 47.3% > F_compact 31.1%**. Read: **width dominates** the spread (the largest net
tops it, the smallest lags badly — and F_compact had the *most* epochs yet lost, so it's
capacity not training-time); the **siamese variants land mid-pack, slightly below the
same-size standard A** — the cleanest ±siamese comparison is D-vs-A (same trunk + a
siamese front-end), where the front-end is a small net negative, consistent with the
front-end being an information bottleneck whose data-efficiency upside is muted in a
data-rich (150k-row) regime. **Caveats:** mid-training, unequal epochs, champion scales as
an approximation, n=1 — not a converged verdict. Catalog row:
`nn_models/REGISTRY.md` (`300k_6arch_sweep`).

**Converged results (the headline).** The three standard archs A/B/C were then resumed to
convergence on Spot (warm-start from each `best.pt` at lr 3e-4, early-stop patience 12;
converged best val-MSE **B_wide 0.5376 / C_deep 0.5432 / A 0.5452**). The siamese variants
D/E and the compact F were left mid-training. A **converged 4-model round-robin that adds
the deployed champion `joint_gelu_rand_240k`** (438 games/pair — one of 8 idx-chunks was
uniformly dropped by a `while read` trailing-newline bug, no bias; 800 sims, mix α=0.9,
champion scales) gives:

| Model | overall win% | vs champion |
|---|---|---|
| **B_wide** (512×512→256) | **65.1%** | **73.2%** |
| A_baseline (256×256→128) | 52.8% | 63.9% |
| C_deep (256×256×256→128) | 50.0% | 66.8% |
| champion (joint_gelu_rand_240k) | 32.0% | — |

**All three 300k-trained archs beat the deployed champion decisively, and the champion is
last.** Because **A_baseline is literally the champion's architecture** (256×256→128) trained
on the new corpus, its 63.9% win **isolates the corpus effect** — the champion's own
1600-sim self-play data yields a much stronger model than the 240k it was trained on.
**B_wide's width adds ~9 points on top** (73.2% vs champion, 58% vs A at equal sims); depth
(C_deep) ≈ A, so width > depth here.

**Equal-wall-clock A-vs-B (converged, 1000 games each).** Because B_wide's 512² trunk is
~1.76× A's forward cost, a fair-compute comparison hands B proportionally fewer sims:
B_wide wins **54.6%** at A@800/B@455 and **57.0%** at A@1408/B@800 — so its edge **survives
compute-normalization** (down only modestly from ~58% at equal sims). B_wide's advantage is
genuine strength-per-compute, not just more FLOPs.

**Status: B_wide is a strong new-champion candidate, NOT yet promoted.** The clean
confirmation before deploying is a B_wide-vs-champion run with **each model's own measured
value_scale** (the round-robin used the champion's scale for all four — fair between the new
archs but an approximation for the absolute champion comparison). The controlled D-vs-A
(±siamese, converged) is still open, since D/E were left mid-training.

> **Finalize perf note (load-bearing for any large sweep).** `shared_dataset.py`'s
> `_split_mask` computed the train/val/test split by constructing a fresh
> `np.random.default_rng(...)` **per row** over the millions of value rows — ~20 min of
> single-threaded finalize on a 150k-game corpus, paid on every box start and resume. Now
> deduped to one RNG per *unique game seed* (a game's rows share a seed; mirrors the
> already-fast `_splits_of`): identical split output, ~seconds. Caught via `py-spy dump` on
> a "mysteriously slow" box — see `CLOUD_RUNBOOK.md` "Diagnosing a repeated failure."
