...
- **MCTS scaffolding** (`agricola/agents/mcts.py`): vanilla UCT + first-play-urgency, a DAG with a transposition table keyed on `GameState`'s hash, leaf-evaluation via the V3 heuristic margin (no rollouts), and macro-enumeration for Fencing (`MacroFencingAction`). Consumes the strict-restriction wrapper. At 200–500 sims with a *heuristic* leaf it loses ~3–5 points to that heuristic standalone — but with the value NN as leaf it beats NN + 1-turn lookahead (early result). See **`MCTS_DESIGN.md`**.
- **Value neural network** (`agricola/agents/nn/`): the first slice — a supervised value network trained on heuristic self-play data to predict the terminal score margin — runs end-to-end (data-gen pipeline → ~170-feature encoder → versioned schema → model/training loop → `NNAgent`). Early results make `NNAgent` the strongest agent to date; the policy head and the full AlphaZero-style self-play loop are still ahead. See **`FIRST_NN.md`**. *(Parts of the NN code are in-progress / uncommitted as of this writing.)*
- **Documentation restructure (2026-05-30):** `CLAUDE.md` reorganized around the project's phases (Foundations → engine → agent → cards); deep engine mechanics moved to **`ENGINE_IMPLEMENTATION.md`**; the old ~50-row status table dropped.
...
## C++ self-play data-gen — 40k games + trace-replay speedup (2026-06-10)

Generated the first at-scale C++ self-play datasets and fixed a replay-path performance footgun found while doing so.

### Data generated
Two C++ MCTS self-play runs at the production config (500 sims, **`c_uct=0.5`**, T=1.0, PUCT/FLATTEN/full-legality, NN value leaf + combined policy, `data_version=3`): **`cpp_selfplay_10k`** (base_seed 5,000,000) and **`cpp_selfplay_30k`** (base_seed 6,000,000) — **non-overlapping seed ranges**, verified against every existing run. Both fully verified: contiguous game_idxs, 100% distinct games, π + `root_value` on every decision, and a clean `validate_dataset` invariant pass. The `c_uct` default was changed `1.4 → 0.5` in both generators (`generate_selfplay_data{,_cpp}.py`; landed in the resumability commit). Data dirs are gitignored/regenerable from their `metadata.json`.

### Bug caught and fixed — `replay_trace` Φ-build pathology (SPEEDUPS S14, commit 18ec06a)
C++ data-gen is two phases: native C++ writes a trace (phase 1); Python `replay_trace` re-runs the action list through the engine to rebuild `GameRecord`s (phase 2). Phase 2 was unexpectedly slow on a *fraction* of games — profiling pinned it on `_build_phi` (the `PARETO_OPT_LEVEL` Φ build) called ~1.35M times in **one** game (24 s), via `replay_trace`'s per-step `legal_actions` call. Root cause: that `legal_actions` was used **only** to decide which states are non-singleton decisions worth snapshotting — never to validate the trace (`step` applies recorded actions unconditionally) — and the Φ build, designed to amortize over the many cap-queries MCTS makes on a farm shape, is pure cost under replay's one-touch-per-state access. Fix: MCTS self-play traces already record a `visit_distribution` **only** on non-singleton decisions, so "entry has π" is set-identical to the `legal_actions` singleton test (verified → byte-identical `GameRecord`s); replay uses that and skips `legal_actions` (value-only traces fall back). **Worst-case game 24,119 ms → 5 ms** (~4,800×); a 30k-game phase-2 replay finished in ~3 min. The clean `validate_dataset` pass — which independently re-derives `legal_actions` per snapshot — is the cross-check that the π-presence shortcut records exactly the right decisions. Notably, the C++ MCTS itself never paid this: it uses the native level-0 `can_accommodate` enumeration (no Φ build — that optimization exists only to mask Python interpreter overhead), so the 24 s Python-replay game was generated by C++ in ~3 s.

### Housekeeping
Pruned four redundant/throwaway C++ runs (`cpp_ab_pergame` — content-identical A/B twin of `cpp_ab_batch`; `cpp_mcts_selfplay_400` — strict subset; `cpp_pipeline_smoke` — 8-game/64-sim smoke; `cpp_selfplay_bench_48` — raw worker-count benchmark traces, not GameRecords), keeping `cpp_ab_batch` as the largest. Redundancy verified by content signature before deletion.
