...
as a before_action_space auto: eligible when the space being used holds at least
1 wood in its `accumulated` Resources. Forest is the normal activator (it
accumulates wood each round); the clay/reed/stone spaces are hooked too because a
card can deposit wood onto them -- the `.accumulated.wood >= 1` gate means the
auto fires only when the space actually holds wood, so a wood-less clay/reed/stone
use never triggers it. NOTE: wood lives in `.accumulated` (a Resources), NOT the
scalar `.accumulated_amount`. On a qualifying use, place a flat 1 food (regardless
of how much wood the space held) on the NEXT round space via the deferred-goods
...
def _eligible(state: GameState, idx: int) -> bool:
    ...
    if space_id not in SPACES:
        return False
    return get_space(state.board, space_id).accumulated.wood >= 1
