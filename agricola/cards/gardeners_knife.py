...
the cell (``cell.grain > 0`` / ``cell.veg > 0``), not by ``cell_type`` alone: an
unsown FIELD holds neither and counts as neither. A sown field holds grain XOR
veg (``_execute_sow`` fills a cell with 3 grain OR 2 veg, never both), so the two
counts never overlap. The grant is asymmetric — food per grain-field, grain per
veg-field — so the two outputs are not transposed.
"""
