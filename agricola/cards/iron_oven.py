"""Iron Oven (minor improvement, E63; Ephipparius Expansion; Food Provider).
...
Cost: 3 Stone. Prerequisite: none. VPs: 2. Not passing.
...
1. **The baking rate (`register_baking_spec_extension`).** For ANY Bake Bread
   action the owner takes, contribute a `(1, 6)` spec — at most 1 grain per
   action at 6 food/grain (cap 1 encodes "convert exactly 1 grain"). The greedy
...
def _baking_spec(state: GameState, player_idx: int) -> list[tuple]:
    """"Convert exactly 1 grain into 6 food" for ANY Bake Bread action the
    owner takes: a (cap 1, rate 6) source while the card is in play. Cap 1
...
# Cost 3 stone; no prerequisite; 2 printed VP.
register_minor(
    CARD_ID,
    cost=Cost(resources=Resources(stone=3)),
    vps=2,
    on_play=_on_play,
)
