...
def breeding_food_gained(
    ...
    Per type: if breeding fired (pre >= 2) AND the newborn was kept (post >= 3),
    the pre-breed removals = (pre + 1 - post), all converted to food. Otherwise
...
def breeding_frontier(
    ...
    for candidate in feasible:
        if not any(dominates(other, candidate) for other in feasible):
            frontier.append((candidate, breeding_food_gained(pre, candidate, rates)))
