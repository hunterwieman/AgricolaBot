import agricola.cards.simple_oven  # noqa: F401  (registers the card; not in __init__ yet)
# Tests for Simple Oven (minor improvement, E64; Ephipparius Expansion).
#
...
# "Bake Bread" action."  Cost 2 Clay; no prerequisite; 1 printed VP.
#
# A BAKING IMPROVEMENT (Clay/Stone Oven family). Two standing baking seams plus a
# one-shot, declinable on-build bake grant:
#   (1) register_baking_spec_extension  -> a (cap 1, rate 3) source for ANY bake
...
# Grain Utilization bake at 3/grain for an oven-less owner; composition with a
...
from agricola.cards.simple_oven import (
    CARD_ID,
    _baking_spec,
...
def test_ovenless_owner_bakes_at_grain_utilization():
    ...
    assert r.grain == 0 and r.food == _N           # 1 grain -> 3 food
...
def test_composes_with_fireplace():
    ...
    rate 3): one bake converts via both — the oven's grain first (higher rate),
    ...
    assert one.players[0].resources.food == _N     # oven (rate 3) fires first

    two = step(s, CommitBake(grain=2))
    r = two.players[0].resources
    assert r.grain == 0 and r.food == _N + 2       # 3 (oven) + 2 (Fireplace)
...
def test_exactly_one_grain_cap():
    ...
    s = step(s, CommitBake(grain=1))
    r = s.players[0].resources
    assert r.grain == 4 and r.food == _N           # exactly 1 grain -> 3 food
