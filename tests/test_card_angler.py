...
qualifies; 3+ does not), snapshotted by a before_action_space auto into the
CardStore; the grant is an OPTIONAL after_action_space trigger on the fishing
host that pushes a PendingMajorMinorImprovement (the Merchant composite-push
idiom), gated on the composite having a legal child right now.
"""
...
def test_registration():
    ...
    assert "angler" in auto_ids                          # the snapshot auto
...
# ---------------------------------------------------------------------------
# The snapshot doesn't leak across uses
# ---------------------------------------------------------------------------

def test_snapshot_does_not_leak_across_uses():
    # First use at 3 food: not offered (and the 3 lands in the CardStore).
    cs, cp = _state(res=Resources(clay=2), food_on_fishing=3)
    cs = _use_fishing(cs)
    assert not _angler_offered(cs)
    cs = step(cs, Stop())                    # end the turn
    assert cs.players[cp].card_state.get("angler", None) == 3

    # Later use at 2 food: the fresh snapshot overwrites the stale 3 -> offered.
    sp = fast_replace(get_space(cs.board, "fishing"),
                      workers=(0, 0), accumulated_amount=2)
    cs = fast_replace(cs, board=with_space(cs.board, "fishing", sp),
                      current_player=cp)
    cs = _use_fishing(cs)
    assert cs.players[cp].card_state.get("angler", None) == 2
    assert _angler_offered(cs)
...
def test_opponent_use_is_atomic_and_grants_nothing():
    ...
    # No snapshot was written for either player.
    assert out.players[cp].card_state.get("angler", None) is None
