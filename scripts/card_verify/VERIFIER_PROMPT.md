...
| `S-SOR` | `start_of_round` + `register_start_of_round_hook` (or hosting driven by a `future_rewards` schedule for one-shot deferred effects). |
| `S-HFIELD` | `harvest_field` + `register_harvest_field_hook`. Mandatory effects use `register_auto`; an optional "you may" effect uses `register` (surfaced at the per-player `PendingHarvestField` choice host, variant-expandable — Stable Manure is the exemplar). An optional effect implemented as a mandatory auto is a finding. |
