...
const items = await pipeline(CARDS,
  ...
  (impl,c) => {
    ...
    return agent(
      ...
      'STEPS: (1) ~/miniconda3/bin/python scripts/card_text.py \"'+c.name+'\" — verbatim text+errata+clarifications. (2) Read agricola/cards/'+c.card_id+'.py + tests/test_card_'+c.card_id+'.py. (3) Check vs text: timing (each-time-you-use=before unless text says after), optionality (you-may=declinable register; mandatory=register_auto), exact thresholds/counts (== vs >=, banded vs cumulative), majors via board.major_improvement_owners, animal grants via market gained-bump not p.animals, NO \"/\" alt-cost or minor play-variant treated as pay-both, and what real-game path the TEST does NOT cover (tautology check). RETURN ONLY the JSON object (3 fields). verdict wrong/suspect must put the precise bug/risk in issues.',
