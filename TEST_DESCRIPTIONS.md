...
### Card-game (Phase 3) — build-fence cost-modifier card tests

The build-fence cost-modifier cards (COST_MODIFIER_DESIGN.md §9): the three free-fence sources (per-action budget / per-edge positional / persistent on-card pool) and the two fence-grant shapes (optional granted action, mandatory restricted grant). Each file verifies the card path AND that the Family game is unaffected.

- **`tests/test_cards_build_fence_cost.py`** — the Cards deferred-tally build-fence cost path: Hedge Keeper free fences, Millwright-on-fences (the running-total conversion + the settle payment menu), Rammed Clay (clay→wood substitution splits), Briar Hedge (board-perimeter frees), and the free-fence-aware legality + placement guard.
- **`tests/test_cards_field_fences.py`** — Field Fences: the granted Build Fences action + its field-adjacency discount, grant-scoped (the discount applies only within the grant); the optional choose-or-decline wrapper (`PendingGrantedBuildFences`); and the larger legal set available under the grant.
