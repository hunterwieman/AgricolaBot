...
| `CARD_DEFERRED_PLANS.md` | **LIVE decision surface for deferred cards**: the defer clusters (grouped by blocker), concrete shared-infrastructure build proposals that would unblock whole clusters at once, and the open user questions. Where a deferring session records its cluster + proposal (CARD_ENGINE_IMPLEMENTATION.md §7). |
