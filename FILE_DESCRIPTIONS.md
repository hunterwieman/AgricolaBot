...
- **`CommitHarvestConversion(conversion_id: str, use: bool)`** — commit one once-per-harvest conversion decision at `PendingHarvestFeed`. `conversion_id` is a key in `HARVEST_CONVERSIONS` (joinery / pottery / basketmaker, plus future card-registered ids). `use=True` fires the conversion (pays `spec.input_cost`, produces `spec.food_out`, applies against `pending.food_owed` with surplus going to supply, invokes `spec.side_effect_fn` if any). `use=False` records the decision without firing. Either way, the conversion_id is added to `player.harvest_conversions_used` so the enumerator no longer offers it for the rest of this harvest's FEED. Dispatched with `auto_pop=False`; the pending stays on top to host further craft decisions and the final `CommitConvert`.
- **`CommitConvert(grain: int, veg: int, sheep: int, boar: int, cattle: int)`** — commit the player's chosen goods-to-food conversion configuration at `PendingHarvestFeed`. Fields hold CONSUMED amounts (subtracted from the player's supply) — contrast with `CommitAccommodate` / `CommitBreed`, which hold post-event-state counts. The CONSUMED convention fits because the values are bounded by per-good caps in `food_payment_frontier` and `(0,0,0,0,0)` always means "consume nothing" regardless of player state. The legality enumerator constructs `CommitConvert` by inverting REMAINING-goods tuples from `harvest_feed_frontier` (consumed = player_max - remaining). After commit: surplus food goes to supply, remaining `food_owed` becomes begging markers (assigned by `_execute_convert`, not by `Stop`, preserving the Stop-only-pops convention), and `pending.conversion_done` is set True. Dispatched with `auto_pop=False`; the trailing `Stop` is the explicit exit.
...
- **`PendingHarvestFeed(player_idx, initiated_by_id, food_owed, conversion_done=False)`** — `PENDING_ID = "harvest_feed"`. One per player during HARVEST_FEED; `initiated_by_id="phase:harvest_feed"`. Hosts trigger-style opt-in sub-decisions (the three craft majors via `CommitHarvestConversion`) followed by one main `CommitConvert`. `food_owed` is set at push time to `max(0, need - p.resources.food)` after pre-debiting the spent food (the "Cannot withhold food tokens" rule, RULES.md Feeding Phase); decreases as crafts fire and `CommitConvert` lands. `conversion_done` gates Stop legality — `Stop` is legal only after `CommitConvert`. No `triggers_resolved` / `TRIGGER_EVENT` yet (Task 5D precedent — natural future events: `before_harvest_feed`, `after_harvest_feed`).
...
- `_execute_harvest_conversion(state, player_idx, commit)` — Task 7. Applies one once-per-harvest conversion decision on `PendingHarvestFeed`. Records the decision (both `use=True` and `use=False`) by adding `commit.conversion_id` to `player.harvest_conversions_used`. If `use=True`: pays `spec.input_cost`, produces `spec.food_out` food (applied against `pending.food_owed` with surplus going to supply), invokes `spec.side_effect_fn` if present. Does NOT pop (`auto_pop=False`); the pending stays on top to host further craft decisions and the final `CommitConvert`.
- `_execute_convert(state, player_idx, commit)` — Task 7. Applies the player's chosen goods-to-food conversion on `PendingHarvestFeed`. `commit.{grain, veg, sheep, boar, cattle}` are CONSUMED amounts (subtracted from supply). `food_produced` computed via `cooking_rates` 4-tuple; splits into `food_consumed_by_owed` (applied against `pending.food_owed`) and `food_surplus` (added to supply). Any remaining owed becomes begging markers (assigned here, not at Stop, preserving the Stop-only-pops convention). Sets `pending.conversion_done=True` and `pending.food_owed=0` via `replace_top`. Does NOT pop; trailing `Stop` is the explicit exit.
...
- **`_initiate_harvest_feed(state)`** — Task 7. Push one `PendingHarvestFeed` per player, ordered so the starting player's frame is on top. Pre-debits food per the "Cannot withhold food tokens" rule (RULES.md Feeding Phase): `spent = min(need, p.resources.food)` is debited upfront; the remaining `food_owed = need - spent` (with `need = 2*people_total - newborns`) becomes `pending.food_owed`. Push order: non-starting player pushed first (bottom of stack), starting player pushed second (top). When the starting player Stops, the non-starting player's frame becomes top automatically. Exposed standalone so tests can construct a FEED-only state without running FIELD mechanics.
