...
- **Phase 3 — Cards (and maybe 4-player).** Implement the full card system, then repeat the
  Phase 2 agent process for the richer game. **The card engine is built; ~320 of the 840-card
...
and phase hooks (start-of-round, the harvest timing-window ladder). **~320 of the 840-card catalog** (Revised base
...
- **The rest of the catalog** — the remaining ~520 cards (many blocked on the deferred-cluster
...
called with a pool of **all implemented cards** (~320; live census via the `OCCUPATIONS`/`MINORS` registries) so each player is
...
- **~520 of the 840 catalog cards** — unimplemented or deferred (clusters + build proposals in
...
AgricolaBot/
    ...
    agricola/                   # THE ENGINE PACKAGE
        ...
        cards/                  # card framework (specs, triggers, cost_mods, capacity_mods, schedules, harvest_conversions, harvest_windows, display) + ~320 card modules
