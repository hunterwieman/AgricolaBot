...
search machinery (c0) has landed** (POLICY_PUCT_DESIGN.md). **Four heads are now trained** — three
fixed-vocab (`placement` 25-way, `choose_subaction` 8-way, `commit_build_major` 14-way) plus the
first **pointer head** `animal_frontier` (score-the-legal-set over the CommitBreed + CommitAccommodate
Pareto frontiers), with unweighted + AWR variants (see `POLICY_HEAD.md` + `nn_models/REGISTRY.md`).
The **`make_policy_fn` combiner** assembles these into the full `policy_fn` MCTS consumes: it works
over the full legal set and dispatches per decision type (fixed head / pointer head / uniform over the
cell-priority-filtered set for the no-signal spatial cell commits / uniform over full legal), so the
prune lives entirely in the policy. Still ahead: the `harvest_feed` pointer head (CommitConvert), a
CommitSow head, fencing, and PUCT consumption/eval of the priors. The full design — input encoding,
supervision target, data pipeline, open questions — is in
**`FIRST_NN.md`**.
...
AgricolaBot/
    ...
    agricola/                       # Game engine package.
        ...
        agents/                     # Agent implementations: random + heuristics. Built atop the engine's pure `step` / `legal_actions` interface.
            ...
            nn/                     # NN value-function infrastructure (subpackage). Schema, recording, and encoder are torch-free so data-generation scripts don't pay the import cost; dataset / model / training / agent import torch and must be imported explicitly (not re-exported from `__init__.py`). See FIRST_NN.md §11.1 for the file-by-file rationale.
                ...
                policy_heads.py     # `DecisionHead` spec + the `HEADS` registry (placement / choose_subaction / commit_build_major) — fixed-vocab heads (owns/vocab/target_index/legal_mask). ALSO the `PointerHead` spec + `POINTER_HEADS` registry (`animal_frontier`: CommitBreed + CommitAccommodate) for variable-cardinality Pareto frontiers — owns/candidate_dim/enumerate_candidates (re-derives the engine frontier with a small action-delta per candidate)/candidates/candidate_features/target_position. The factored policy: dataset/model/training/prior are all head-driven, so adding a head is a new spec here, not new modules. Torch-free. See POLICY_HEAD.md.
                ...
                policy.py           # `policy_prior(state, model)` (fixed heads) + `pointer_prior(state, model)` (pointer heads) + `NO_PRIOR`, and `make_policy_fn(models)` — the full `policy_fn(state, legal) -> {action: prior}` MCTS/PUCT consumes. Works over the FULL legal set, dispatching by decision type: fixed head / pointer head / uniform over the cell-priority-filtered set (spatial cell commits — no encoder signal) / uniform over full legal (the rest). The prune lives entirely in the policy. Imports torch.
    ...
    scripts/                        # Out-of-tree utilities — profiling, benchmarking, tuning. Re-runnable; not imported by `agricola/` or `tests/`. Used to produce / update PROFILING.md and the tuned-config JSONs in `tuned_configs/`.
        ...
        nn/                         # NN-specific scripts (subdirectory to keep NN tooling separate from general utilities). All are re-runnable CLIs; the underlying libraries live in `agricola/agents/nn/`.
            ...
            train_policy_pointer.py # Thin CLI over `agricola.agents.nn.policy_pointer_training.train_pointer` (`--head {animal_frontier}`, `--loss-weight {none,awr}`, `--value-ckpt`, `--awr-clip`, `--init-from`). Default `--run-dir` = the three hidden-info runs (pointer-head labels are wrapper/forcing-fix-invariant, so all DATA_VERSION-2 data is valid — not just hidden_info_v2_10k). See POLICY_HEAD.md.
