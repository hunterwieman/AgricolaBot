...
- `scripts/nn/generate_training_data.py` — orchestrates heuristic self-play via the 8-config ensemble from `tuned_configs/DATA_GEN_ENSEMBLE.md`. Multiprocessing pool, deterministic plan, bimodal per-agent temperature draws, atomic per-game pickle writes, resume-on-existing per §6.4. **`--p0-fixed-config`** (added C21) pins seat 0 to one config and turns `--approved-configs`/`--config-weights` into a P1-only sampling pool — the asymmetric scheme behind `e14_hardmix_1k` (NN in seat 0, opponents weighted toward the NN's weaknesses). NN seats via `nn:<checkpoint>` specs.
...
- `scripts/nn/retention_eval.py` (added C20) — post-hoc **retention** sweep: encode a fixed held-out slice of a *broad-distribution* run dir once, then compute raw-margin MAE for any list of checkpoints (e.g. every saved epoch of a fine-tune), with a baseline model for the reference line. The instrument that exposes self-play *forgetting* — each fine-tune's own val split can't, since it measures fit-to-the-new-distribution. `predict_margin` denormalizes per-model so MAE-in-points is comparable across checkpoints with different NormStats.
- `agricola/agents/nn/training.py` (+ `scripts/nn/train_first.py`) gained **`--l2sp <λ>`** (L2-SP anchor `λ·‖θ−θ₀‖²` toward the `--init-from` weights — a trust region, distinct from wd's pull toward 0; requires a warm-start) and **`--save-all-epochs`** (write `epoch_NNN.pt` every epoch so checkpoints can be selected by head-to-head play rather than the unreliable val-MSE-best). Both added C20.
...
**Version history:** `1→2` — hidden-information refactor (`ActionSpaceState.round_revealed: int` → `revealed: bool`), changing the embedded `GameState` (see `HIDDEN_INFO_DESIGN.md`). `2→3` — MCTS self-play recording: `DecisionSnapshot` gained two optional fields, `visit_distribution` (raw root visit counts π, the AlphaZero policy target, stored unnormalized at τ=1) and `root_value` (the search's P0-frame value estimate), both `None` for heuristic / `NNAgent` records. The `None` defaults make v2 records structurally a subset of v3, so the only thing gating v2 data out of the v3 loader is the version check — re-stamp a `GameRecord`'s `data_version` to 3 to load old runs under v3 code (the fields then read `None`). Self-play data is produced by `scripts/nn/generate_selfplay_data.py` via `agricola/agents/nn/selfplay_recording.py`.

### 10.5 Encoded-vector cache (per-run-dir)

**Status: implemented** (`build_datasets_chunked(..., use_cache=True)` / `train(use_cache=True)` / `--use-cache`). Measured: 5k run-dir build 163 s (miss, encode + write 329 MB) → **1.9 s (hit) = 85× speedup**; one cache serves all three target modes (outcome-mode build from the same cache in 2.1 s, no re-encode). The implementation follows this spec, with one deviation noted inline below (the split is seed-hash unconditionally in the chunked builder, not just under the cache). Encoding is the slow part of every training run (`encode_state` over millions of descriptors, single-threaded Python; ~10-15 min for the 55k set before epoch 1). Today nothing persists the encoded arrays — every training run re-encodes from the pickles, even when it reuses the same games. This cache persists the encoding so re-runs skip it.
...
*Fix.* Normalize each head's leaf by its own σ (a persisted per-model `value_scale` + `MCTSSearch.leaf_value_scale`; see §10 / commit) so all heads feed UCB on unit Q-scale. The point of normalizing is that **c becomes head-independent** — one c serves all three heads — and it is therefore *not* itself σ-scaled: c is a plain exploration hyperparameter in unit-Q space. The value used here, `0.0612`, is merely the unit-space *image* of the classical raw-margin constant (`1.4/σ_margin ≈ 1.4/23`); note the conversion uses σ_margin for **every** head, which is the tell that this σ is a one-time *anchor* constant — **not** a per-head normalization of c. Only the *ratio* of c to the Q-spread matters: UCB's argmax is invariant to a global rescale of `Q + c·√(…)`, so "normalize V by σ **and** use `1.4/σ_margin`" is a single units choice, not two corrections — equivalent to skipping normalization and using raw per-head `c = 1.4·σ_head/σ_margin`. **Caveat:** `0.0612` is the *unswept* classical constant inherited through this anchor, not a tuned value — once V is normalized, c should be calibrated directly in unit-Q space (a sweep), not assumed to be `1.4/σ`. The 6-game smoke confirmed only that the broken *over-exploration* was gone — winprob-MCTS scored a normal 21-28 (not −1), matchup 3-3 — not that `0.0612` is optimal.
...
**C20 — Self-play fine-tuning beats the parent; L2-SP anchoring hurts (round 1).** *Fine-tune the champion `M_82k_warmM62k` on its own 1-turn self-play data (`M82k_nn70_t2_5k`: 5000 games, P0/P1 each 70% M82k / 30% t2, bimodal T), and A/B an L2-SP anchor.* Three arms, all warm-started from M82k, lr 3e-4, dropout 0.2, wd 0, `--save-all-epochs` (checkpoints selected by gameplay, not val MSE — two new training flags, §10.1): plain (`ftA`), L2-SP λ=1e-3 (`ftB1`), λ=1e-2 (`ftB2`). L2-SP = `λ·‖θ−θ₀‖²` toward the warm-start weights (a trust region around M82k), distinct from wd's pull toward 0.

- **Plain wins decisively.** `ftA` epoch 14 beats M82k **+5.07 (304-95-1, n=400, 76.2%)**, with a clean inverted-U over epochs (e4 +3.63 → e14 +5.32 → e25 +2.74). The student fine-tuned on its own trajectories beats the teacher — *expert iteration in miniature*. Mechanism: M82k was trained on heuristic-dominated data, so it is mis-calibrated on the states NN-vs-NN play actually visits; the self-play data (exact terminal-margin labels) recalibrates it there.
- **Both anchored arms LOSE to M82k** (ftB1 −1.79 / 38-61; ftB2 −3.32 / 32-68 — more anchor → worse). The drift L2-SP suppresses is *beneficial adaptation* toward the deployment distribution, so anchoring a *base* model against it is counterproductive. (Even the near-frozen ftB2 is *worse* than M82k, not equal — it refit its NormStats on the self-play data while the anchor blocked the weights from adapting to that new normalization: worst of both worlds.)
- **Retention instrument (new: `scripts/nn/retention_eval.py`).** Broad-distribution MAE on a held-out slice of `standard_bimodal_5k`, with M82k as baseline — the missing yardstick, since each arm's own val split only measures fit-to-the-new-distribution. Plain forgets *progressively* (+0.38 e1 → +0.80 e25 over M82k); anchored arms stay *flat* (~+0.34–0.40). So the anchor measurably limits forgetting — but **drift ≠ harm**: the "forgetting" was the price of beneficial specialization. Broad MAE is again anti-correlated with strength.
- **Generalization (8-config ensemble, regular legality, 800 games).** `ftA` e14 = **94.4%** vs M82k's 96.4% — still crushes the ensemble (not pure overfit-to-M82k), but a ~2-pt drop concentrated in the V3 configs it drifted from (alphas_gen_1 84%, gen_7 90%). Takeaway: self-play fine-tuning genuinely improves NN-vs-NN play; the cost is mild de-calibration vs heuristics. **Not promoted** (see C22). `ftA` e14 is the parent of C21.

**C21 — Hard-opponent mining + a gentle anchor patches the weakness (round 2).** *From `ftA` e14, fine-tune on data mined for e14's weaknesses, and test whether the anchor's verdict flips now that we refine an already-adapted model.* New generator capability (`--p0-fixed-config`, §10.1): an **asymmetric** seat scheme — P0 pinned to e14, P1 drawn from a pool weighted by each opponent's **win-rate against the NN** (alphas_gen_1 heaviest at 26%; the two 0%-win configs, gen16/gen47, excluded). This produced `e14_hardmix_1k` (1000 games, 30% e14 self-play + 70% hard heuristics). Two arms, warm-started from e14, trained on `e14_hardmix_1k` + the original 5k: plain (`ft2_plain`) and L2-SP λ=1e-4 (`ft2_l2sp1e4`, gentle, anchored to e14).

- **Hard-mix only helps *with* the anchor.** `ft2_plain` e10 over-specializes: it is the **strongest pure NN-vs-NN player** (beats e14 +2.59; wins the round-robin below) but its ensemble **regresses to 88.0%** and the *target* weakness gets worse (alphas_gen_1 84→80, gen_7 90→77). `ft2_l2sp1e4` e8 ("R_e8") beats e14 **+2.92** (most of any candidate), **holds** the ensemble at **93.7%** (≈ e14's 94.4%), and **patches** the weak spots (alphas_gen_1 84→**94**, gen_7 90→**94**).
- **The anchor verdict flips from C20.** Here the gentle anchor (λ=1e-4 — the untested gentle end; 1e-3/1e-2 froze the fit in C20) is *helpful*: it keeps the refinement from drifting off the heuristic distribution, so the hard examples improve generalization instead of trading it away. Anchoring hurts when *adapting a base model* (C20) but helps when *refining an already-adapted model* (C21).
- **3-model round-robin** (e14 / `ft2_plain` e10 / `ft2_l2sp1e4` e8; n=100, strict, argmax): standings **plain_e10 64.8% > R_e8 50.5% > e14 34.8%** (transitive: plain_e10 beats R_e8 58-40 and e14 70-29; R_e8 beats e14 60-40). So the strongest *pure NN-vs-NN* player (plain_e10) and the best *generalist* (R_e8) are different models — the specialization tension made explicit.
- **Not promoted** (C22). `ft2_l2sp1e4` e8 is the best all-rounder; `ft2_plain` e10 the strongest pure player.

**C22 — The self-play loop has plateaued by the objective yardstick (hall of mirrors).** *Diagnosis, not a new training run.* The two yardsticks contradict: **head-to-head-vs-parent** says large gains (e14 beats M82k +5.07; R_e8 beats e14 +2.92), while the **fixed 8-config ensemble panel** — the only cheap, uncontaminated, external reference — is essentially **flat-to-down across the whole chain** (M82k 96.4% → e14 94.4% → R_e8 93.7% → plain_e10 88.0%). They can't both be objective improvement.

- **Why:** each model is fine-tuned *on its parent's own self-play distribution* and then evaluated *against that parent*, so the head-to-head gain is partly **learning to exploit the parent's specific value landscape**, not playing objectively better Agricola. Classic self-play hall-of-mirrors. plain_e10 is the clearest case (best at beating NNs, worst vs the fixed panel).
- **Root cause:** the loop has **no teacher stronger than the student** — data is the net's own 1-turn play labeled by outcomes, which mostly reinforces existing valuations / learns the specific opponent. AlphaZero climbs because *MCTS* is a stronger-than-net teacher; 1-turn self-play has no such signal, so it specializes/exploits rather than objectively improves.
- **Methodological consequences:** (1) gate on the **fixed ensemble panel** (and/or a fixed held-out reference), **not** head-to-head-vs-parent, which is contaminated by construction; (2) the panel is also *saturated* (all models 88–96%), so it can't finely rank the top — the principled tiebreaker is **MCTS-leaf strength** (the actual deployment use, and much harder to exploit than 1-ply), affordable for *evaluation* (a few hundred games) even though MCTS is too slow for data-scale generation; (3) to actually break the plateau you need a **cheap stronger-than-1-turn teacher** for data (2–3 ply lookahead, or a tiny ~20–30-sim MCTS budget used only for labeling), not more 1-turn self-play rounds.
...
- **Datasets on disk** (`data/nn_training/runs/`, gitignored):
  ...
  - `M82k_nn70_t2_5k` — 5000-game NN self-play (P0/P1 each 70% `M_82k_warmM62k` / 30% t2, base_seed 15M); the C20 fine-tune data.
  - `e14_hardmix_1k` — 1000-game hard-opponent-mined run (base_seed 16M): P0 pinned to `ftA` e14, P1 = 30% e14 self-play + 70% heuristics weighted by win-rate-vs-NN (`--p0-fixed-config`); the C21 fine-tune data.
- **Trained checkpoints:** the authoritative catalog is **`nn_models/REGISTRY.md`** (one row per checkpoint + per-model details). The C20–C22 fine-tunes (`ftA_plain_M82k_nn70t2`, `ftB1`/`ftB2`, `ft2_plain_e14_hardmix6k`, `ft2_l2sp1e4_e14_hardmix6k`) are all recorded there; none is promoted — `M_82k_warmM62k` remains champion (`nn_models/best`).
...
- Promotion gating: a **`nn_models/best.{pt,meta.json}` pointer now exists** (a copy of the current champion's checkpoint — currently `M_82k_warmM62k`), and `play_web.py` loads it by default, so the browser UI always plays the champion; promoting = overwrite that pair (no code change). What's still missing vs `tuned_configs/v3_best.json` is the **automated regression-gated update rule** — promotion is currently manual (hand-copied + `REGISTRY.md` updated by hand), not auto-gated on a head-to-head threshold.
...
### 13.6 Self-play fine-tuning, the plateau, and objective evaluation (from C20–C22)

- **The 1-turn self-play loop has plateaued in *objective* strength.** Fine-tuning a model on its own self-play data produces big *head-to-head-vs-parent* gains (C20 +5.07, C21 +2.92) while the fixed ensemble panel stays flat-to-down (96.4→94.4→93.7%). The head-to-head metric is **contaminated** — the fine-tune trains on, then is tested against, the same parent distribution, so it learns to *exploit* the parent rather than play objectively better. **Open:** confirm with **MCTS-leaf** evaluation (harder to exploit; the real deployment use; affordable for eval even though MCTS is too slow for data-scale generation) — does R_e8-as-MCTS-leaf actually beat M82k-as-MCTS-leaf, or does the edge evaporate under search?
- **Breaking the plateau needs a teacher stronger than the student.** 1-turn self-play has none (data = the net's own play, labeled by outcome), so it specializes/exploits instead of improving. The affordable middle path between 1-turn (too weak) and 300-sim MCTS (too slow for data): **2–3 ply lookahead** for data generation, or a **tiny ~20–30-sim MCTS** used *only* for labeling. Untested.
- **The objective yardstick itself is saturated.** All NNs beat the 8-config ensemble 88–96%, so it can't finely rank the top models. Need either a stronger fixed panel (e.g. MCTS-backed heuristics) or an Elo ladder over many checkpoints, to get a ranking that isn't pairwise-vs-parent.
- **Anchoring (L2-SP) is regime-dependent** (C20 vs C21): it *hurts* when adapting a base model to a new distribution (suppresses beneficial drift) but *helps* when refining an already-adapted model against over-specialization. The gentle end (λ=1e-4) is the useful setting; 1e-3+ freezes the fit. Open: is there a principled way to pick λ, or to schedule it across rounds?
- **Hard-example mining by matchup-frequency is weak on its own** (C21): feeding more *games* vs a hard opponent mostly oversamples a matchup the NN already wins (so the terminal-margin labels reinforce winning), not the *losses*. It only patched the weakness when paired with an anchor. Open: loss-weighted or margin-weighted sampling that actually oversamples the hard *outcomes*.
