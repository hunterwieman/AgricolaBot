...
> - **Multi-shot builders** (`PendingBuildStables` / `PendingBuildRooms` / `PendingBuildFences`) —
>   **DONE.** All three were folded into the uniform before/after host model (a real `phase` field,
>   `Proceed` as the work-complete flip once the counter `>= 1`, firing `after_build_<x>` autos via
>   `_enter_after_phase`, the trailing `Stop` a pure pop): rooms/stables in the sub-action-hook
>   follow-on, `build_fences` in the Shepherd's Crook refactor. So `before_/after_build_rooms`,
>   `_build_stables`, and `_build_fences` all fire now (the build-fence/stable/room cards — Hedge
>   Keeper, Asparagus Gift, Loppers, Lumberjack, Wall Builder, Pigswill, Shepherd's Crook, … — are
>   unblocked on this axis). This was the last *action-layer* firing gap; the action/sub-action hook
...
> - **Step 7** — deferred cards (Organic Farmer, Mini Pasture, Acorns Basket) last. (**Shepherd's Crook
>   is now DONE** — its blocker, a before/after host on `build_fences`, was built; see the Deferred-cards note.)
>
...
- **The multi-shot builders** (`PendingBuildStables`, `PendingBuildRooms`, `PendingBuildFences`)
  were *subsequently* folded into the same uniform host model (rooms/stables in a follow-on,
  `build_fences` in the Shepherd's Crook refactor): each gained a `phase` field and flips to its
  after-phase on an explicit `Proceed` (the multi-shot has no single commit to flip on), the
  trailing `Stop` a pure pop. No Stop-terminated sub-action exception remains (Side Job, a
  Family-only space, is the lone Stop-terminated frame).
...
**Roughcaster · Junk Room · Mining Hammer · Bread Paddle · Dutch Windmill · Shepherd's Crook**
...
gated on `round_number in {5,8,10,12,14}` (stateless). **Shepherd's Crook** is now **DONE**
(`agricola/cards/shepherds_crook.py`): a `before_/after_build_fences` automatic pair that snapshots
the pasture decomposition at the start of the Build Fences action and grants 2 sheep per new ≥4-space
pasture in the net diff — see the Deferred-cards note (its blocker, the build_fences before/after host,
was built for it).
...
- **Shepherd's Crook** (fence hook) — **DONE** (`agricola/cards/shepherds_crook.py`,
  `tests/test_cards_shepherds_crook.py`). "Each time you fence a *new* pasture covering ≥4 spaces, get
  2 sheep on it" is evaluated on the **net result of the whole Build Fences action, not
  per-`CommitBuildPasture`** — because splitting a rules-atomic action into commits is a *tractability*
  artifact, and a card keying on the action must see it as the rules do. The blocker (the build_fences
  host had no before/after phase) was removed by giving `PendingBuildFences` the uniform before/after
  host treatment (a `phase` field, `Proceed`-flip, the trailing `Stop` a pure pop — Python + C++ +
  NN-relabel, differential gates green; see SUBACTION_HOOK_REFACTOR.md). **Implementation:** a
  `before_build_fences` automatic effect snapshots the enclosed cells into the `CardStore`; an
  `after_build_fences` automatic effect diffs against the final pasture decomposition and grants 2
  sheep per pasture whose cells are *all* newly-enclosed AND span ≥4. The 2 sheep always fit (a ≥4
  pasture has capacity ≥8 and is empty), so it is a no-decision automatic grant. **Maintainer ruling
  (confirmed):** if you fence a new 6-space area and split it into a 4 and a 2 in the same action, the
  undivided 4-space piece *does* grant (the 2 does not) — which is exactly why the grant is computed
  once at action-end, not per-commit. (General principle: rules-atomic actions we split for
  tractability should fire card effects on the net, at action-end.)
...
7. **Deferred cards** (Organic Farmer, Mini Pasture, Acorns Basket) last. (**Shepherd's Crook done** —
   its blocker, a before/after host on `build_fences`, was built; see the Deferred-cards note.)
