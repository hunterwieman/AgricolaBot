...
- ⬜ **A40 Potter's Yard** · cost: 1 Wood,1 Reed · prereq: At Most 7 Unused Farmyard Spaces · **⚠ REVISIT before implementing (ruling 87, 2026-07-29)**: its "get the clay and you can immediately exchange it for 2 food" makes plowing a PRODUCER mid-chain — the guarded-fire certificates (plow_maker/plow_hero's `_can_plow_twice`, Drill Harrow's preserve pair, Thresher's simulated buy) summarize chains but do not SEARCH them, so a mid-chain rescue line is invisible to every gate (wrongly refused) and the chain's own branches (decline the exchange; plow the wrong cell) can strand a must-act host. Implementing this card (or any plow-chain producer) requires the chain-scoped completability filtering first — the bounded mechanical-move search recorded in ruling 87 — and the user must be informed.
...
- ⛔ **B75 Wood Workshop** · cost: 1 Clay · prereq: 1 Occupation · **WONTFIX — BANNED (2026-07-29)**
  ...
  - **Un-implemented 2026-07-29 (was ✅):** on the official ban list (too strong) — external knowledge, not recorded in the repo's card data, so it was implemented and dealt in good faith before the ban was known. Module + test archived to `archive/banned_cards/`, deregistered from `cards/__init__.py` (census −1 minor). Its before-window income grant is also why the improvement-window auto tripwire exists (`tests/test_improvement_window_autos.py`, ruling 87): the affordability gates read the current state, and an auto on `before_build_major`/`before_play_minor` lands its goods only after the gates have answered.
...
- ✅ **D17 Drill Harrow** · cost: 1 Wood
  ...
  - **Ruling 87 (2026-07-29), two corrections**: (1) the raise frame gained its preserve filter (`_preserve_sow`, shared with eligibility) — the shipped version's unfiltered menu could offer a seed-burning bundle and strand the mandatory sow (executed soft-lock, regression-pinned in `test_card_drill_harrow_preserve.py`); the hand-rolled reserve-one-seed eligibility was deleted with it, also fixing its over-strict card-field-sow corner. (2) The route itself is now sow-capability: `register_sow_extension` — no empty field but a plowable cell + a board seed + preserving-payable 3 food makes Grain Utilization / Cultivation placeable (and choose-sow enterable, the fire then mandatory), inherited by every sow-granting card's gate and by Thresher's simulated post-buy state (the priced two-card chain).
...
- ⬜ **D60 Large Pottery** · cost: 1 Clay,1 Stone · prereq: Return the Pottery · **⚠ REVISIT before implementing (ruling 82, 2026-07-26; instruction updated 2026-07-29 under ruling 87)**: its anytime Clay→2-Food conversion breaks the engine's ONE-WAY payment assumption — goods convert INTO food, never the reverse, and only crops/animals feed a work-phase raise (`_liquidatable_to` is built on this; every affordability and preserve argument relies on it). The rebuilt `tests/test_liquidation_disjointness.py` WILL fail when this lands. Do NOT weaken its asserts, add this card to its `_SKIP`, or re-derive safety locally — the original instruction here ("re-derive the one-direction safety gates in `full_peasant.py` / `large_scale_farmer.py`") is OBSOLETE: those gates were removed by ruling 87's uniform fee shape, which already asks every destination's gate on the true post-payment state. Instead: STOP and inform the user — the user decides how the payment machinery extends to two-way conversion.
...
- ✅ **C112 Thresher** · [1+] · ruling 84 (2026-07-27: food price rebuilt on the raise shape) · ruling 87 (2026-07-29: space-enable extension — the buy can be what makes a Grain Utilization / Cultivation placement (or jump destination) legal, evaluated by simulating the buy per payment case and asking the real capability predicates; when the buy is the sole admitting route the fire is mandatory, emergent from the host exits; Farmland deliberately unextended — grain never enables a plow)
